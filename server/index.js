const {
  ...
  app.post('/api/customers/:customer_id/reservations', async(req, res, next)=> {
    try {
      res.status(201).send(await createReservation({customer_id: req.params.customer_id, ...req.body}));
    }
    catch(ex){
      next(ex);
    }
  ...
  const init = async()=> {
    ...
    app.listen(port, ()=> {
      ...
      console.log(`curl localhost:${port}/api/customers/${res2.customer_id}/reservations/${res2.id} -X DELETE`);
      console.log(`curl localhost:${port}/api/reservations`);
      console.log(`curl localhost:${port}/api/customers/${moe.id}/reservations -X POST -d '{"restaurant_id": "${diner.id}", "party_count": 9, "date": "11/11/2024"}' -H "Content-Type:application/json"`);
    });
